...
| Plunder Frenzy momentum system + AAA feedback pass (shatter debris, drop banners, gold pop) | ✅ |
| Hall of Legends collection log (jarls, rarities, curses, Legend score) | ✅ |
| Rune Storms weekend events (5 rotating, deterministic, no backend) | ✅ |
| Valhalla Ascension meta-prestige (Marks + 4 permanent godly boons) | ✅ |
| Tests (`npm test` → 206 checks, 0 errors) | ✅ |
...
| 4.2 | 🔲 **Screenshots** (min 2, rec 3–8) | Phone: 16:9 or 9:16, min 320px, max 3840px JPEG/PNG | Play Console → Main store listing | **Starter set included:** `screenshots/01_splash.png … 11_valhalla.png` (true 1080×2340 renders, unique art per region + boss lair). Regenerate any time with `npm i @napi-rs/canvas && node scripts/screenshot.js`. For live captures, run on a real device and use `adb exec-out screencap -p > shot.png`. |
